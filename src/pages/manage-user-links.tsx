...
export const Page_ManageUserLinks = () => {
  ...
  const dbMatches = useQuery<TMatch[]>({
    queryFn: async () => {
      return await api
        .get<{ matches: TMatch[] }>("/manage-user-links/available-matches")
        .then((res) => res.data.matches);
    },
    queryKey: ["manageUserLinks.matches"],
  });

  const userLinks = useQuery<{ links: TUserLink[] }>({
    queryKey: ["manageUserLinks.userLinks"],
    queryFn: async () => {
      return await api
        .get<{ links: TUserLink[] }>("/manage-user-links/get-user-links")
        .then((res) => res.data);
    },
  });
  ...
  const allUserIDs = useMemo(() => {
    return Array.from(
      new Set(userLinks.data?.links?.map((link) => link.user_id) ?? [])
    );
  }, [userLinks.data?.links]);
  ...
  const deleteLink = useMutation({
    mutationFn: async ({ id }: { id: string }) => {
      return await api.post("/manage-user-links/delete-link", {
        id,
      });
    },
    onSuccess: () => {
  ...
  const deleteLinks = useMutation({
    mutationFn: async ({ ids }: { ids: string[] }) => {
      return await api.post("/manage-user-links/delete-links", {
        ids,
      });
    },
    onSuccess: () => {
  ...
  const currLinks = userLinks.data?.links
    ...
    ?.filter((link) => {
      if (userIdQuery) return link.user_id === userIdQuery;
          ...
          <SelectContent>
            ...
            {allUserIDs.map((id, idx) => (
              <SelectItem value={id} key={idx}>
                {allUsers.data?.find((u) => u.id === id)?.username}
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
        ...
        <TableBody>
          {currLinks
            ...
            .map((link) => {
              ...
              const user = allUsers.data?.find((u) => u.id === link.user_id);
              if (!user) return null;
